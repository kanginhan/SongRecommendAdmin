<template>
  <div class="pl-3 pt-3">
    ...
    <v-layout>
      ...
      <v-flex>
        <v-btn color="primary" @click="addSingSong">Add Song</v-btn>
      </v-flex>
      <v-flex>
        <v-spacer></v-spacer>
      ...
      <v-flex>
        ...
        >
          <template v-slot:items="props">
            <td>
              <a href="#" @click="showLyrics(props.item)">{{ props.item.songId }}</a>
            </td>
        ...
        <v-dialog v-model="dialog" width="500">
          <v-card>
            <v-card-title class="headline grey lighten-2" primary-title>{{selectedSong.title}}</v-card-title>
...
export default {
  ...
  methods: {
    ...
    addSingSong: function() {
      this.$axios
        .get(`${UrlConfig.word.addSingSong}?songId=${this.songId}`)
        .then(response => {
          alert(response.data);
          this.initialize();
        });
    ...
    initialize: function() {
      this.$axios.get(UrlConfig.word.getListBaseSongs).then(response => {
    ...
    analyze: function() {
      ...
      if (respond) {
        ...
        this.$axios.get(UrlConfig.word.analyzeLyrics).then(response => {
